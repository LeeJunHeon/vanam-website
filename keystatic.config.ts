  ...
  collections: {
    ...
    materials: collection({
      ...
      schema: {
        ...
        name: fields.text({ label: '정식 명칭 (영문, 선택)', description: '예: Vanadium Dioxide — 비우면 화학식이 표시됩니다' }),
        name_ko: fields.text({ label: '정식 명칭 (한글, 선택)', description: '예: 이산화바나듐' }),
        image: fields.image({
          label: '자료 이미지 (선택)',
          description: 'A4 세로 자료 권장 — 팝업에서 원본 비율 그대로 크게 표시됩니다.',
          directory: 'src/assets/materials',
          publicPath: '/src/assets/materials/',
        }),
        description: fields.text({ label: '상세 설명 (영문, 선택)', multiline: true }),
        description_ko: fields.text({ label: '상세 설명 (한글, 선택)', multiline: true }),
